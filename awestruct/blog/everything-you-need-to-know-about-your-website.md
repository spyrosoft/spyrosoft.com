...
Good registrars are very rarely good hosts and vice versa. I recommend registering your domains separately from your host.
...
For shared php website **hosting** [DreamHost](https://www.dreamhost.com/) is the best option available. A single account offers unlimited domains, email accounts, linux users (awesome), databases, bandwidth, and free SSL certificates.
...
A very large security issue arises when cPanel is used. All of the websites are placed together in the same directory. This makes all of the websites only as secure as the weakest website. In general it is a good idea to avoid cPanel altogether for this reason.

**Horror Story Time!**

For example, some friends of mine had an account at BlueHost. There were ten or so WordPress websites. One of the sites got hacked, then all of the sites got hacked. The hacker began sending out massive amounts of spam and replaced much of the website content with malicious content. After a while BlueHost shut down the account. Which is reasonable. However, BlueHost makes no distinction between web hosting accounts and email accounts. Everyone's email went down. Additionally, it turned out that the backup restoration button did absolutely nothing. Backups are a "feature" that costs an extra monthly fee. Most of the website content is gone forever.
...
* **PHP** - One of the most commonly used programming languages for the web. For example Facebook and Twitter use it. Pretty much any forum you've ever seen uses it. Which is remarkable considering how poorly designed the language is. Unlike the other languages mentioned here, for every single page load the following happens:
...
Rather than create a great deal of code every single time a website is created, it makes more sense to create a **platform** which can be reused over and over again for lots of different websites.

The most popular platform for the every day person's website is WordPress.

New languages are created when programmers find they do not have the correct tools to accomplish a certain, usually specific, type of goal. Platforms are created when the community using a language finds they do not have the correct tools to accomplish a certain, usually specific, type of goal. A platform is a collection of code which attempts to make common tasks easier. The tradeoff is an additional learning curve.
...
## The Beast That Is DNS

Haha, just kidding.

## Technology Analysis Diagram

## Keywords

Registrar, Domain Name, Host, Server, Database, Table, Column, Row, Cluster